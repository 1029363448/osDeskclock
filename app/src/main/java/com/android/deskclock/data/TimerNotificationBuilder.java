    ...
    public Notification build(Context context, NotificationModel nm, List<Timer> unexpired) {
        ...
        if (count == 1) {
            if (running) {
                ...
                String addMinuteAc=null;
                if (WrapperManager.isMtkPlatform()) {
                    addMinuteAc = TimerService.ACTION_ADD_MINUTE_TIMER_UNEXPIRED;
                } else if (WrapperManager.isQualcommPlatform()) {
                    addMinuteAc = TimerService.ACTION_ADD_MINUTE_TIMER;
                }

                final Intent addMinute = new Intent(context, TimerService.class)
                        .setAction(addMinuteAc/*TimerService.ACTION_ADD_MINUTE_TIMER_UNEXPIRED*/)
                        .putExtra(TimerService.EXTRA_TIMER_ID, timer.getId());
