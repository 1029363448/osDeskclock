...
public final class AlarmStateManager extends BroadcastReceiver {
    ...
    ///M: [ALPS03424585] Changing the buffer value from 15 in AOSP code to 5  @{
    public static final int ALARM_FIRE_BUFFER = WrapperManager.isMtkPlatform() ? 5 : 15;
    ///@}
    ...
    private static void updateNextAlarm(Context context) {
        ...
        /// M: [ALPS03269543] [Power off Alarm] It just be used for power off alarm.
        /// If the next alarm is overdue, @ {
        if(WrapperManager.isMtkPlatform()) {
            ...
            /// M: Avoid unneccessarily calling reset in normal case
            if (isBootFromPoweroffAlarm) {
                resetServiceForAlarm(context, nextAlarm);
            }
        }
    }
    ...
    public static void setMissedState(Context context, AlarmInstance instance) {
        ...
        /// M: [ALPS03269543] [Power off Alarm] If PowerOffAlarm timeout set it to
        /// Snoozed state else Missed state @{
        if(WrapperManager.isMtkPlatform()) {

            if (PowerOffAlarm.bootFromPoweroffAlarm()) {
                Calendar currentTime = Calendar.getInstance();
                Calendar timeoutTime = instance.getTimeout();
    ...
    public static void fixAlarmInstances(Context context) {
       ...
       /// M: If bootup is due to alarm, do not fix alarms as this will also set the
       /// fired alarm as missed alarm and we would not find any relevant alarm later @{
        if (WrapperManager.isMtkPlatform()) {
            if (PowerOffAlarm.bootFromPoweroffAlarm()) {
                return;
            }
        ...
        public void scheduleInstanceStateChange(Context context, Calendar time,
            ...
            if (WrapperManager.isMtkPlatform()) {
                if (instance.mAlarmState == AlarmInstance.PREDISMISSED_STATE) {
                    ...
                    PowerOffAlarmPlatformMTK.cancelPoweroffAlarm(context);//DC-163
                }
                /// @}
            }
        }
    ...
    public static void setPoweroffAlarm(Context context, AlarmInstance nextAlarm) {//DC-163
        if (nextAlarm != null && /*PowerOffAlarm.canEnablePowerOffAlarm()*/PowerOffAlarmPlatformMTK.canEnablePowerOffAlarm()) {
    ...
    public static void resetServiceForAlarm(Context context, AlarmInstance nextAlarm) {
        ...
        PendingIntent pendingIntentAlarm = PendingIntent.getService(context, nextAlarm.hashCode(),
                stateChangeIntentAlarm, PendingIntent.FLAG_UPDATE_CURRENT);
